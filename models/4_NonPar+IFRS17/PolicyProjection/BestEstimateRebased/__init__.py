...
def EXPS_CLAIM_DTH(t):
    ...
    if t <= 0 or t > Input.MAX_PROJ_LEN():
        ...
        return Actuals.EXPS_CLAIM_DTH(t)
    else:
        try:
            basis = Input.AssumptionsMatrix["Inflation"][scen]
        except KeyError:
            basis = "Previous"
        ...
        return flag * Input.Exp_Claim_A["Fixed"] * POLS_DTH(t) * Input.INFL_FAC(t+1, basis)
...
def EXPS_CLAIM_MAT(t):
    ...
    if t <= 0 or t > Input.MAX_PROJ_LEN():
        ...
        return Actuals.EXPS_CLAIM_MAT(t)
    else:
        try:
            basis = Input.AssumptionsMatrix["Inflation"][scen]
        except KeyError:
        ...
        return flag * Input.Exp_Claim_A["Fixed"] * POLS_MAT(t) * Input.INFL_FAC(t, basis)
...
def EXPS_CLAIM_SURR(t):
    ...
    if t <= 0 or t > Input.MAX_PROJ_LEN():
        ...
        return Actuals.EXPS_CLAIM_SURR(t)
    else:
        try:
            basis = Input.AssumptionsMatrix["Inflation"][scen]
        except KeyError:
    ...
    ## Inflation adjustment for EOM
        ...
        return flag * Input.Exp_Claim_A["Fixed"] * POLS_LAPSE(t) * Input.INFL_FAC(t+1, basis)
...
def EXPS_MAINT(t):
    ...
    if t <= 0 or t > Input.MAX_PROJ_LEN():
        ...
        e           = Input.Exp_Maint_A["Fixed"]/12 * Input.INFL_FAC(t, basis)
...
def POLS_DTH(t):
    ...
    if t <= 0 or t > Input.MAX_PROJ_LEN():
        ...
        return Actuals.POLS_DTH(t)
    else:
        try:
            basis = Input.AssumptionsMatrix["Mortality"][scen]
        except KeyError:
            basis = "Previous"

        return IND_ACTIVE_POL(t) * POLS_IF(t) * Input.MORT_RATE_MLY(t, basis)
...
def POLS_MAT(t):
    ...
    if t <= 0 or t > Input.MAX_PROJ_LEN():
        ...
        return (Input.DUR_M(t) == Input.POL_TERM_M() + 1) * (POLS_IF(t) - POLS_LAPSE(t) - POLS_DTH(t))
...
def PV_PREM(t):
    ...
    if t <= 0 or t > Input.MAX_PROJ_LEN():
        ...
        return PREM_PAYBL_M(t) + PV_PREM(t+1)/(1 + Input.DISC_RATE_M(t, "Locked"))
...
def PV_DTH_BEN(t):
    ...
    if t <= 0 or t > Input.MAX_PROJ_LEN():
        ...
        return (DTH_BEN(t) + PV_DTH_BEN(t+1)) / (1 + Input.DISC_RATE_M(t, "Locked"))


def PV_OTH_BEN(t):
    """Present value of surrender, income and maturity benefits"""

    if t <= 0 or t > Input.MAX_PROJ_LEN():
        return Input.pd.Series(0, index = Input.MP().index)
    else:
        disc_fac = 1 + Input.DISC_RATE_M(t, "Locked")
        return (INCM_BEN(t) + MAT_BEN(t) + (SURR_BEN(t) + PV_OTH_BEN(t+1))/disc_fac)
...
def PV_COMM(t):
    ...
    if t <= 0 or t > Input.MAX_PROJ_LEN():
        ...
        return COMM_INIT(t) + COMM_REN(t) + PV_COMM(t+1)/(1 + Input.DISC_RATE_M(t, "Locked"))
...
def PV_EXPS(t):
    ...
    if t <= 0 or t > Input.MAX_PROJ_LEN():
        ...
        disc_fac = 1 + Input.DISC_RATE_M(t, "Locked")
        return (EXPS_MAINT(t) + EXPS_CLAIM_MAT(t)
                + (EXPS_CLAIM_DTH(t) + EXPS_CLAIM_SURR(t) + PV_EXPS(t+1))/disc_fac)
...
def PV_EXPS_ACQ(t):
    ...
    if t <= 0 or t > Input.MAX_PROJ_LEN():
        ...
        return EXPS_ACQ(t) + PV_EXPS_ACQ(t+1)/(1 + Input.DISC_RATE_M(t, "Locked"))
...
def PV_RA_CF(t):
    ...
    if t <= 0 or t > Input.MAX_PROJ_LEN():
        ...
        return (RA_MORT(t) + PV_RA_CF(t+1)) / (1 + Input.DISC_RATE_M(t, "Locked"))
...
def NON_ONEROUS():
    """Boolean for whether the individual policy is non-onerous"""
...
Actuals = ("Interface", ("..", "Actuals"), "auto")
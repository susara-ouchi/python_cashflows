...
def ACCM_FV(t):
    ...
    if t <= 0 or t > Proj.MAX_PROJ_LEN():
        ...
        return (ACCM_FV(t-1) + AV_PP_AT(t, "BEF_PW_LB")) * Proj.IND_ACTIVE(t)
...
def ALLOC_CHARGE_RATE(t):
    ...
    if t <= 0 or t > Proj.MAX_PROJ_LEN():
        ...
        n = Input.AllocCharge["Projection"].index.max()
        dur = list(Input.np.minimum(Proj.DUR_Y(t), n))
        return Input.pd.Series(list(Input.AllocCharge["Projection"][dur]), index = Proj.MP().index)
...
def AV_PP_AT(t, timing):
    ...
    .. rubric:: END
    Account value at the end of the time `t` after deducting partial withdraw
    and crediting loyalty bonus
    ...
    if t <= 0 or t > Proj.MAX_PROJ_LEN():
        ...
        return AV_PP_AT(t-1, "END") * Proj.IND_ACTIVE(t)
...
def CHG_ADMIN(t):
    ...
    if t <= 0 or t > Proj.MAX_PROJ_LEN():
        ...
        return Input.np.minimum(AV_PP_AT(t, "BEF_FEE"), Input.np.minimum(Cap, Charge)) * Proj.IND_ACTIVE(t)
...
def CHG_ALLOC(t):
    ...
    if t <= 0 or t > Proj.MAX_PROJ_LEN():
        ...
        flag = Proj.IND_ACTIVE(t) * ((Proj.DUR_M(t) - 1)/12 == Proj.DUR_Y(t) - 1) * (Proj.DUR_M(t) <= Proj.PREM_TERM_M())
...
def CHG_FMC(t):
    ...
    if t <= 0 or t > Proj.MAX_PROJ_LEN():
        ...
        return (AV_PP_AT(t, "BEF_INV") + INV_INC_PP(t)) * Input.UF_FMC["FMC"]/12 * (1 + Input.ST_rate["FMC"]) * Proj.IND_ACTIVE(t)
...
def CHG_MORT(t):
    ...
    if t <= 0 or t > Proj.MAX_PROJ_LEN():
        ...
        return ((SAR(t)* mort * scale)/12* (1+Input.ST_rate["Mortality Charge"]) * (AV_PP_AT(t,"BEF_FEE") > 0)) * Proj.IND_ACTIVE(t)
...
def INV_INC_PP(t):
    ...
    if t <= 0 or t > Proj.MAX_PROJ_LEN():
        ...
        inv_rate_mth = (1 + Input.YieldCurve["Unit Interest Rates"][dur]) ** (1/12) - 1
        return (inv_rate_mth * AV_PP_AT(t, "BEF_INV")) * Proj.IND_ACTIVE(t)
...
def LOYALTY(t):
    ...
    if t <= 0 or t > Proj.MAX_PROJ_LEN():
        ...
        flag = (Proj.DUR_Y(t) == Proj.DUR_M(t)/12) * Proj.IND_ACTIVE(t)
...
def PREM_TO_AV_PP(t):
    ...
    if t <= 0:
        ...
        flag = Proj.IND_ACTIVE(t) * (Proj.DUR_M(t) < Proj.PREM_TERM_M())
        return (Proj.PREM_PP() - CHG_ALLOC(t)) * ((Proj.DUR_M(t)-1)/12 == Proj.DUR_Y(t)-1) * flag
...
def PW(t):
    ...
    if t <= 0 or t > Proj.MAX_PROJ_LEN():
        ...
        return (Proj.MP()['ann_prem'] * Input.np.minimum(Proj.DUR_Y(t), Proj.PREM_TERM_M()//12) * PW_RATE(t)/12) * Proj.IND_ACTIVE(t)
...
def PW_RATE(t):
    ...
    if t <= 0 or t > Proj.MAX_PROJ_LEN():
        ...
        dur = list(Input.np.minimum(Proj.DUR_Y(t), n))
        return Input.pd.Series(list(Input.PartialWithdrawal["Projection"][dur]), index = Proj.MP().index)
...
def ST_ADMIN(t):
    ...
    if t <= 0 or t > Proj.MAX_PROJ_LEN():
        ...
        return Input.np.maximum(0, (CHG_ADMIN(t)/(1+Input.ST_rate["Allocation/Admin Charge"])*Input.ST_rate["Allocation/Admin Charge"])) * Proj.IND_ACTIVE(t)
...
def ST_ALLOC(t):
    ...
    if t <= 0 or t > Proj.MAX_PROJ_LEN():
        ...
        return Input.np.maximum(0, (CHG_ALLOC(t)/(1+Input.ST_rate["Allocation/Admin Charge"]) * Input.ST_rate["Allocation/Admin Charge"])) * Proj.IND_ACTIVE(t)
...
def ST_FMC(t):
    ...
    if t <= 0 or t > Proj.MAX_PROJ_LEN():
        ...
        return Input.np.maximum(0, (CHG_FMC(t)/(1+Input.ST_rate["FMC"])*Input.ST_rate["FMC"])) * Proj.IND_ACTIVE(t)
...
def ST_MORT(t):
    ...
    if t <= 0 or t > Proj.MAX_PROJ_LEN():
        ...
        return Input.np.maximum(0, (CHG_MORT(t)/(1 + Input.ST_rate["Mortality Charge"])*Input.ST_rate["Mortality Charge"])) * Proj.IND_ACTIVE(t)
